...
// Adjusted instanceConfig struct without SSL references
...
var httpc2definition = c2structs.C2Profile{
	Name:             "tor-http",
	Author:           "@cammclain",
	Description:      "Uses Tor to send HTTP Get/Post messages for connectivity",
	IsP2p:            false,
	IsServerRouted:   true,
	ServerBinaryPath: filepath.Join(".", "tor-http", "c2_code", "mythic_http_server"),
	ConfigCheckFunction: func(message c2structs.C2ConfigCheckMessage) c2structs.C2ConfigCheckMessageResponse {
		...
		// Ensure callback_port is provided
		if _, ok := message.Parameters["callback_port"]; !ok {
			response.Success = false
			response.Error = "Failed to get callback_port attribute"
			return response
		}
	
		// Ensure callback_host is provided and matches Tor hidden service format
		if suppliedHost, ok := message.Parameters["callback_host"]; !ok {
			response.Success = false
			response.Error = "Failed to get callback_host attribute"
			return response
		} else {
			callbackHost := suppliedHost.(string)
			if !strings.HasSuffix(callbackHost, ".onion") {
				response.Success = false
				response.Error = "Callback host must be a Tor hidden service ending with .onion"
				return response
			}
		}
	
		// Custom logic here to ensure the configuration aligns with Tor's requirements
		// This is where you can add more checks relevant to your setup
	
		response.Message = "Configuration check passed. Ensure your Tor hidden service is correctly set up."
		return response
	},
